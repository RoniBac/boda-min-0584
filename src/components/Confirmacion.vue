  ...
  <section class="confirmacion" v-if="data">
    ...
    <div v-if="estado === 'confirmar'" class="rsvp-box">
      ...
      <div v-if="pasoConfirmacion === 'decision'">
        ...
        <button
          class="btn"
          @click="seleccionarAsistencia('si')"
        >
          Asistiré
        </button>
        ...
        <button
          ...
          @click="seleccionarAsistencia('no')"
        >
          No asistiré
        </button>
...
// 🎯 SELECCIONAR OPCIÓN
// =======================
const seleccionarAsistencia = async (tipo) => {

  asistencia.value = tipo

  // SI ASISTE → CONFIRMA DIRECTO
  if (tipo === "si") {
